-- Gym Crew - Supabase schema + helpers
-- Run in Supabase SQL editor (in order). Safe to re-run most parts.

create extension if not exists pgcrypto;

-- Enums (as text checks to keep it simple / portable)
...
create table if not exists public.gym_groups (
  ...
  -- routine_url stores the Storage object path (e.g. routines/{groupId}/routine.pdf)
  routine_url text,
...
-- Manual check-in approval (any other member)
...
-- Award monthly winner badge (idempotent)
...
-- gym_groups: members can read, admins can update/delete
...
-- group_members: members can read. Admins can manage. User can leave (delete self row).
...
-- gym_locations: members can read, admins manage
...
-- check_ins: members read; user can insert own; updates restricted (handled via RPC)
...
-- manual_approvals: members can read; inserts handled by RPC but allow member insert for transparency
...
-- badges: members of group can read; user can read own
...
-- invites: admins can read/manage; members cannot see token list by default
